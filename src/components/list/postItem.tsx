      ...
      >
        <CardBody>
          <div className="w-full flex flex-col p-4 sm:flex-row sm:items-center sm:gap-12 sm:justify-start justify-center">
            <div
              className={`w-full h-[100px] sm:max-w-[240px] sm:max-h-[200px] ${
                +origindata.id % 2 === 0 ? 'sm:order-1' : 'sm:order-2'
              }`}
            >
              <Link
            ...
            >
              ...
              <p className=" line-clamp-3">{origindata.summary}</p>

              <div className="flex gap-4 items-center flex-wrap text-slate-600">
                <Link href={'#'} className="flex items-center gap-1">
                  <Calendar color="#6F42C1" />
                  <span>
                    {dayjs(origindata.createdAt).format('YYYY-MM-DD HH:mm:ss')}
                  </span>
                </Link>
                <Link href={'#'} className="flex items-center gap-1">
